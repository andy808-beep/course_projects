...
```{r}
# The pattern shows that integrating initial condition uncertainty has little impact on the prediction, with small increase in uncertainty in the early prediction period. The changes match the framework, which suggest that with all other terms remaining linear, the uncertainty grows or declines exponentially over time.
```
...
```{r}
#The new parameter uncertainty in red shows a ci range that is relatively constant over time. The framework suggest that parameter uncertainty does not necessarily increase or decrease over the forecast period. However, it also states that with sampling (MCMC), the uncertainty decline asymptotically to zero, which dwhatid not happen in this forecast.
#Instead of sampling all the parameter at once in N.IP, we could do so repeatedly with each parameter to distinguish their contributions.
```
...
```{r}
#The framework suggest that predictability increases when drivers are predictable. The driver analysis indicates that the uncertainty in precipitation increase over time, which cause the forecast uncertainty to increase in scale. 
```
...
```{r}
#Over time, the driver uncertainty could increase its contribution in the forecast uncertainty and diminish the process error. The framework also suggest that the process error will not decline to zero. 
```
...
```{r}
tau.mean <- mean(params[prow,"tau_site"])
tau1.mc <- 1/sqrt(tau.mean) ## convert from precision to std deviation
aNew1.mc <- rnorm(Nmc,0,tau1.mc)            ## draw out-of-sample predictions of alpha at a new site

N.IPDEA1 <- forecastN(IC=IC[prow,"N[6,30]"],    ## sample IC
                   ppt=ppt_ensemble[drow,],    ## Sample drivers
                   r=params[prow,"r_global"],  ## sample parameters
                   Kg=params[prow,"K_global"],
                   beta=params[prow,"beta"],
                   alpha=aNew1.mc,              ## sample random effect
                   Q=Qmc,                      ## process error
                   n=Nmc)

## Plot run
plot.run()
N.IPDEA1.ci = apply(N.IPDEA1,2,quantile,c(0.025,0.5,0.975))
ecoforecastR::ciEnvelope(time2,N.IPDEA.ci[1,],N.IPDEA1.ci[3,],col=col.alpha(N.cols[5],trans))
ecoforecastR::ciEnvelope(time2,N.IPDE.ci[1,],N.IPDE.ci[3,],col=col.alpha(N.cols[4],trans))
ecoforecastR::ciEnvelope(time2,N.IPD.ci[1,],N.IPD.ci[3,],col=col.alpha(N.cols[3],trans))
ecoforecastR::ciEnvelope(time2,N.IP.ci[1,],N.IP.ci[3,],col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.ci[1,],N.I.ci[3,],col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci[2,],lwd=0.5)
#The change in pattern in unclear. By comparing their means of ci range, the posterior mean approach reduced the overall uncertainty by 10 units. 
```
```{r}
print(mean(N.IPDEA1.ci[3,] - N.IPDEA1.ci[1,]))
print(mean(N.IPDEA.ci[3,] - N.IPDEA.ci[1,]))
```
...
```{r}
#The plot suggests that the random effect, or the site-to-site variability, contributes to the out-of-sample uncertainty dominantly. This underscores the importance of studying individual sites for thier own environmental factors such as nutrient cycle and abiotic interaction. The In-Sample prediction variance is initially represented by the proccess error shifting to the driver uncertainty overtime. The process error could be improved by refining the model structure or reducing measurement errors. The driver uncertainty could be reduced by new data of precipitation, or adding a different driver as supplementary covariate.
```
